...
// 1. Keep third-party layout constants outside component to save recreation memory
const COLORS = ["#eab308", "#16a34a", "#ef4444"];

// 2. LAZY LOAD RECHARTS: This extracts the large charting library out of your main admin bundle chunk.
// It will only load asynchronously when the dashboard mounts.
const LazyChart = React.lazy(() => import("../helper/AdminChartHelper"));
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoading(true);
      await Promise.all([fetchStats(), fetchBookings()]); // Parallel execution for faster load times
  ...
  // 3. FIX PERFORMANCE: Memoize array search filters so typing doesn't freeze the screen
  ...
  // 4. FIX PERFORMANCE: Memoize chart metrics so typing in the search box doesn't rerun filters
  ...
  // 5. MEMORY SAFETY: Automatically revoke preview blobs when changing pictures or unmounting
    ...
    <div className="min-h-screen bg-white relative overflow-hidden pt-28 px-4 xs:px-5 sm:px-6 md:px-8 lg:px-12">
      ...
      <div className="text-center max-w-3xl mx-auto mb-12 xs:mb-16">
        ...
        <h1 className="font-serif text-3xl xs:text-4xl sm:text-5xl md:text-6xl font-bold leading-tight">
          Dashboard<span className="text-burgundy"> Management</span>
        </h1>
      ...
      <div className="mb-4 xs:mb-5">
        <input
          ...
          className="w-full sm:w-96 bg-white border border-gold/20 rounded-full px-4 xs:px-6 py-2 xs:py-3 text-sm xs:text-base shadow-sm focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20"
        />
      </div>
      ...
      <div className="flex flex-wrap gap-2 xs:gap-3 mb-6">
        <button
          ...
          className={`px-4 xs:px-5 py-1.5 xs:py-2 rounded-full text-xs xs:text-sm font-semibold transition-all duration-300 ${filter === "ALL" ? "bg-burgundy text-white shadow-lg shadow-burgundy/20" : "bg-white border border-gold/20 text-charcoal/60 hover:bg-gold/10"}`}>
          All
        </button>
        <button
          onClick={() => setFilter("PENDING")}
          className={`px-3 xs:px-4 py-1.5 xs:py-2 rounded-full text-xs xs:text-sm font-medium transition ${filter === "PENDING" ? "bg-yellow-500 text-white" : "bg-gray-100 text-gray-600"}`}>
          Pending
        </button>
        <button
          onClick={() => setFilter("CONFIRMED")}
          className={`px-3 xs:px-4 py-1.5 xs:py-2 rounded-full text-xs xs:text-sm font-medium transition ${filter === "CONFIRMED" ? "bg-green-600 text-white" : "bg-gray-100 text-gray-600"}`}>
          Confirmed
        </button>
        <button
          onClick={() => setFilter("CANCELLED")}
          className={`px-3 xs:px-4 py-1.5 xs:py-2 rounded-full text-xs xs:text-sm font-medium transition ${filter === "CANCELLED" ? "bg-red-500 text-white" : "bg-gray-100 text-gray-600"}`}>
          Cancelled
        </button>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-gold/10 shadow-lg p-4 xs:p-6 sm:p-8 mb-12 xs:mb-16">
        <h2 className="font-serif text-2xl xs:text-3xl font-bold">
          Gallery<span className="text-burgundy"> Management</span>
        </h2>
              ...
              {filteredBookings.map((b) => (
                <tr key={b._id} className="border-t">
                  ...
                  <td className="p-2 xs:p-3">
                    <span
                      className={`px-2 xs:px-3 py-0.5 xs:py-1 rounded-full text-[8px] xs:text-xs font-medium ${b.status === "Pending" ? "bg-yellow-100 text-yellow-700" : b.status === "Confirmed" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                      {b.status}
                    </span>
                  </td>
                  <td className="p-2 xs:p-3">
      ...
      {/* ANALYTICS SECTION WITH LAZY-LOADED CHART */}
      <div className="mt-12 xs:mt-16 bg-white rounded-3xl border border-gold/10 shadow-lg p-4 xs:p-6 sm:p-8">
        <div className="mb-6 xs:mb-8 text-center">
          <h2 className="font-serif text-2xl xs:text-3xl sm:text-4xl font-bold">
            Booking<span className="text-burgundy"> Analytics</span>
          </h2>
        ...
        <div className="h-[280px] xs:h-[320px] sm:h-[350px] mb-6 xs:mb-8">
          {/* Using React Suspense to isolate chart parsing delays */}
          <React.Suspense
            fallback={
              <div className="h-full w-full flex items-center justify-center text-charcoal/50 text-sm">
                Loading visual analytics data...
              </div>
            }>
            <LazyChart chartData={chartData} COLORS={COLORS} />
          </React.Suspense>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 xs:gap-6">
          <button className="bg-white border border-gold/10 rounded-3xl p-4 xs:p-6 hover:-translate-y-2 hover:shadow-xl transition cursor-pointer text-center">
            <span className="block text-2xl xs:text-3xl mb-2">📅</span>View
            Bookings
          </button>
          <button
            ...
            <span className="block text-2xl xs:text-3xl mb-2">🖼</span>Manage
            Gallery
          </button>
          <button className="bg-white border border-gold/10 rounded-3xl p-4 xs:p-6 hover:-translate-y-2 hover:shadow-xl transition cursor-pointer text-center">
            <span className="block text-2xl xs:text-3xl mb-2">👥</span>Manage
